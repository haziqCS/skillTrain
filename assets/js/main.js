...
document.addEventListener("alpine:init", () => {
  ...
  Alpine.data("headerData", () => ({
    ...
    logout() {
      ...
      window.location.href = "/skillTrain/pages/login.html";
    },
  }));
  ...
  Alpine.data("courseFilter", () => ({
    ...
    get filteredCourses() {
      ...
      return this.courses.filter((course) => {
        ...
        const matchesCategory = Object.values(this.categories).some(
          (val) => val
        )
  ...
  Alpine.data("courseRegistration", () => ({
    ...
    init() {
      ...
      const registeredCourseNames = currentUser.registeredCourses.map(
        (course) => course.name
      );
      ...
      if (courses) {
        ...
        this.availability =
          courses.find((course) => course.name === this.name)?.availability ||
          [];
        // Debugging output
        console.log("Availability for", this.name, ":", this.availability);
        //this.availability = courses.availability.find(c => c.name === this.name);

        this.immediate_registration =
          courses.find((course) => course.name === this.name)
            ?.immediate_registration || [];
        this.registerDate = currentUser.date_registered;
        this.registeredCourses = currentUser.registeredCourses || [];
        this.waitlist = currentUser.waitlist || []; // Ensure this is initialized as an empty array if not found
      ...
      if (currentUser && courses) {
        ...
        const registeredCourse = currentUser.registeredCourses.find(
          (c) => c.name === this.course.name
        );
        ...
        if (registeredCourse) {
          ...
          const courseData = courses.find(
            (c) => c.name === registeredCourse.name
          );
          if (courseData) {
            this.course.immediate_registration =
              courseData.immediate_registration === "Yes";
            this.course.availability = courseData.availability; // Update availability
          }
        }

        // Check if the course is in the waitlist
        const waitlistedCourse = currentUser.waitlist.find(
          (c) => c.name === this.course.name
        );
        ...
        if (waitlistedCourse) {
          ...
          // Find matching course in courses data
          const courseData = courses.find(
            (c) => c.name === waitlistedCourse.name
          );
          if (courseData) {
            this.course.immediate_registration =
              courseData.immediate_registration === "Yes";
  ...
  Alpine.data("userProfile", () => ({
    ...
    dropCourse(course) {
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("dropCourseModal")
      );
      modal.show();
    },
    ...
    confirmDropCourse() {
      ...
      this.registeredCourses = this.registeredCourses.filter(
        (c) => c.name !== this.courseToDrop.name
      );
  ...
  Alpine.data("courseData", () => ({
    ...
    async init() {
      ...
      this.course =
        courses.find((course) => course.name === courseName) || courses[0];
  ...
  Alpine.data("lessonPageData", () => ({
    ...
    get currentLesson() {
      return this.course.lessons
        ? this.course.lessons[this.currentLessonIndex]
        : {};
    },
  }));
});
